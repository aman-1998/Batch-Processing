 ...
 * jobs from running during the build process.
 * 
 ...
 * This command has to be run from inside the target folder. But when we will build the
 * code at that time itself the job will be triggered. So, even if we try to run the the command 
 * to run the job, the job won't get triggered as during build it is already triggered.
 * 
 */
...
//	}
	...
	public CommandLineRunner commandLineRunner() {
		...
		return args -> {
			JobParameters jobParameters = new JobParametersBuilder().addString("Credit Bank Name", "IDFC FIRST BANK")
					.addString("Debit Bank Name", "YES BANK")
					.toJobParameters();
