...
public class SkipBadRecordListenerForReview {
    ...
    public void skipInRead(Throwable th) {
		...
		String errorMsg = (th instanceof FlatFileParseException) 
		  ...
		  : th.getMessage();
		createFile(filePath, "Error in reading from REVIEW table in Oracle DB: " + errorMsg);
    }

    @OnSkipInProcess
    public void skipInProcessor(ReviewOra reviewOra, Throwable th) {
    	String filePath = "C:\\Users\\DELL\\Desktop\\Batch_Processing_With_Spring_Batch_And_Spring_Boot\\"
				  + "Spring_Batch_DB_Migration_Oracle_To_MySql_N41\\"
				  + "Chunk Job\\SimpleStep\\Processor\\"
				  + "SkipedInProcessorBadRecord.txt";

		createFile(filePath, "Error in processing row from REVIEW table in Oracle DB:  " 
								+ reviewOra.toString() + "\nException: " + th.getMessage());
    }

    @OnSkipInWrite
    public void skipInWriter(ReviewMsq reviewMsq, Throwable th) {
    	String filePath = "C:\\Users\\DELL\\Desktop\\Batch_Processing_With_Spring_Batch_And_Spring_Boot\\"
				  + "Spring_Batch_DB_Migration_Oracle_To_MySql_N41\\"
				  + "Chunk Job\\SimpleStep\\Writer\\"
				  + "SkipedInWriterBadRecord.txt";
  	
        createFile(filePath, "Error in Writing to REVIEW table in MySql DB: " + reviewMsq.toString() 
        						+ "\nException: " + th.getMessage());
    }
