...
public class SkipBadRecordListenerForStudentCourse {
    ...
    public void skipInRead(Throwable th) {
		...
		String errorMsg = (th instanceof FlatFileParseException) 
		  ...
		  : th.getMessage();
		createFile(filePath, "Error in reading from STUDENT_COURSE table in Oracle DB: " + errorMsg);
    }

    @OnSkipInProcess
    public void skipInProcessor(StudentCourseOra studentCourseOra, Throwable th) {
    	String filePath = "C:\\Users\\DELL\\Desktop\\Batch_Processing_With_Spring_Batch_And_Spring_Boot\\"
				  + "Spring_Batch_DB_Migration_Oracle_To_MySql_N41\\"
				  + "Chunk Job\\SimpleStep\\Processor\\"
				  + "SkipedInProcessorBadRecord.txt";

		createFile(filePath, "Error in processing row from STUDENT_COURSE table in Oracle DB:  " 
								+ studentCourseOra.toString() + "\nException: " + th.getMessage());
    }

    @OnSkipInWrite
    public void skipInWriter(StudentCourseMsq studentCourseMsq, Throwable th) {
    	String filePath = "C:\\Users\\DELL\\Desktop\\Batch_Processing_With_Spring_Batch_And_Spring_Boot\\"
				  + "Spring_Batch_DB_Migration_Oracle_To_MySql_N41\\"
				  + "Chunk Job\\SimpleStep\\Writer\\"
				  + "SkipedInWriterBadRecord.txt";
  	
        createFile(filePath, "Error in Writing to STUDENT_COURSE table in MySql DB: " + studentCourseMsq.toString() 
        						+ "\nException: " + th.getMessage());
    }
